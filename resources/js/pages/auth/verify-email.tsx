...
export default function VerifyEmail({ status }: { status?: string }) {
    ...
    async function openInBrowser() {
        // TAMBAHKAN KIOSK GENERATE DULU AGAR LANGSUNG RIDIRECT LOGIN SEHINGGA TINGGAL MELAKUKAN VERIFIKASI
        const verificationUrl = `${window.location.origin}/email/verify`;
        await AppLauncher.openUrl({ url: verificationUrl });
    }
            ...
            <div className="space-y-6">
                {isNativeApp ? (
                    <div className="rounded-lg bg-orange-50 p-4 text-sm text-orange-800 dark:bg-orange-900/20 dark:text-orange-400">
                        <p className="font-medium">Terdeteksi Penggunaan Aplikasi</p>
                        <p className="mt-1">
                            Disarankan verifikasi lewat browser, proses verifikasi email lebih stabil saat dibuka di browser perangkat.
                        </p>
                        <Button
                            type="button"
                        ...
                        >
                            Buka halaman verifikasi di browser
                        </Button>
                    </div>
                ) : (
                    <>
