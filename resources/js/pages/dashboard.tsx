...
export default function Dashboard({ attendanceToday, recentVisits, user, serverTime, products = [] }: Props) {
    ...
    useEffect(() => {
        const checkSecurity = async () => {
            ...
            if (!platform) return;
            // Jika dibuka dari Web
            if (platform === 'web') {
                showAlert(
                    "Akses Ditolak",
                    "Bagus Dinamika Nusantara melalui Browser Laptop/PC. Silakan gunakan Aplikasi Mobile Android.",
                    "error",
                    () => {
            ...
            if (platform === 'android') {
                try {
                    const status = await DevMode.isEnabled();

                    if (status.enabled) {
                        showAlert(
                            "Keamanan Terancam",
                            "Terdeteksi penggunaan Lokasi Palsu (Fake GPS). Matikan aplikasi tambahan dan Mode Pengembang untuk melanjutkan.",
                            "warning",
                            () => App.exitApp(),
                            true
                        );
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <AlertModal
                ...
                primaryButtonText={alertConfig.type === 'error' ? 'Keluar' : 'OK Mengerti'}
                disableBackdropClick={alertConfig.isFatal} // Kalau fatal, user gabisa klik background buat nutup
            />
