...
export default function Monitoring({ recentVisits, selectedDate, serverTime }: Props) {
    ...
    const itemsPerPage = 10; // Naikkan untuk lebih banyak data
    ...
    return (
        <div className="min-h-screen bg-blue-50/20 dark:bg-blue-950/10 pb-16 pt-8 px-5 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-7xl space-y-10">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-[11px] md:text-xs font-bold uppercase tracking-[0.15em] text-blue-600 dark:text-blue-400 opacity-80">
                                    {formatDateLong(parseWIB(filterDate))}
                                </p>
                            </div>
                        </div>
                ...
                {totalItems > 0 ? (
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight leading-none mb-1">
                                        Riwayat Kegiatan Seluruh User
                                    </h3>
                                    <p className="text-sm font-medium text-slate-600 dark:text-slate-300">
                                        Menampilkan data <span className="text-slate-900 dark:text-slate-200 font-bold">{startIndex + 1}-{Math.min(endIndex, totalItems)}</span> dari total <span className="text-slate-900 dark:text-slate-200 font-bold">{totalItems}</span> untuk tanggal {filterDate}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="text-center py-10 text-zinc-500">
                        Tidak ada data kegiatan untuk tanggal {filterDate}
                    </div>
                )}
            </div>
            ...
            <div className="fixed bottom-4 right-4 z-50 flex items-center gap-3">
                <a
                    href="/htx20sht7bwy9pgYP4VP7AFhYi6GL5ryffX1LAYMwBbKoo8jA7uBqVbQcmPT1RGkVAtFuh7v4k9xicGxGQMTXZxba3CWFOIoxgw"
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold shadow transition-colors text-sm"
                >
                    <ChevronLeft size={16} />
                    Kembali
                </a>
