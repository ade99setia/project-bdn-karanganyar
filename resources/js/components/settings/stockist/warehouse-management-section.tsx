...
interface Props {
    ...
    warehouseForm: { name: string; code: string; is_active: boolean; address: string; phone: string; receipt_header: string; receipt_footer: string };
    setWarehouseForm: Dispatch<SetStateAction<{ name: string; code: string; is_active: boolean; address: string; phone: string; receipt_header: string; receipt_footer: string }>>;
    ...
    warehouseEditForm: { name: string; code: string; is_active: boolean; address: string; phone: string; receipt_header: string; receipt_footer: string };
    setWarehouseEditForm: Dispatch<SetStateAction<{ name: string; code: string; is_active: boolean; address: string; phone: string; receipt_header: string; receipt_footer: string }>>;
            ...
            <div className="mb-6 space-y-3">
                <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Nama gudang / toko"
                            className="w-full rounded-xl border border-gray-300 bg-white h-12 pl-12 pr-10 py-2.5 text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:focus:border-indigo-400 dark:focus:ring-indigo-400/30 transition-all duration-200"
                        />
                        ...
                        />
                        <button type="button" onClick={() => createFileInputRef.current?.click()} title="Upload foto gudang"
                    ...
                    <div className="flex gap-2">
                        <button type="button" onClick={() => setWarehouseForm((prev) => ({ ...prev, is_active: true }))}
                        ...
                        >Aktif</button>
                        <button type="button" onClick={() => setWarehouseForm((prev) => ({ ...prev, is_active: false }))}
                    ...
                    <button onClick={createWarehouse} disabled={warehouseLoading || !warehouseForm.name.trim() || !warehouseForm.code.trim()}
                ...
                <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
                    <input value={warehouseForm.address} onChange={(e) => setWarehouseForm((prev) => ({ ...prev, address: e.target.value }))}
                        ...
                        className="rounded-xl border border-gray-300 bg-white px-3.5 py-2.5 text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:focus:border-indigo-400 dark:focus:ring-indigo-400/30 transition-all duration-200"
                    />
                    <input value={warehouseForm.phone} onChange={(e) => setWarehouseForm((prev) => ({ ...prev, phone: e.target.value }))}
                        placeholder="No. telepon (opsional)"
                        className="rounded-xl border border-gray-300 bg-white px-3.5 py-2.5 text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:focus:border-indigo-400 dark:focus:ring-indigo-400/30 transition-all duration-200"
                    />
                    <input value={warehouseForm.receipt_header} onChange={(e) => setWarehouseForm((prev) => ({ ...prev, receipt_header: e.target.value }))}
                        placeholder="Header struk (mis: Selamat datang!)"
                        className="rounded-xl border border-gray-300 bg-white px-3.5 py-2.5 text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:focus:border-indigo-400 dark:focus:ring-indigo-400/30 transition-all duration-200"
                    />
                    <input value={warehouseForm.receipt_footer} onChange={(e) => setWarehouseForm((prev) => ({ ...prev, receipt_footer: e.target.value }))}
                ...
                <table className="min-w-full text-sm">
                    <thead className="bg-gray-50 text-left dark:bg-gray-800/60">
                        <tr>
                            ...
                            <th className="px-3 py-3">Alamat / Telp</th>
                            <th className="px-3 py-3">Header / Footer Struk</th>
                                ...
                                <tr key={warehouse.id} className="transition-colors hover:bg-gray-50/50 dark:hover:bg-gray-800/30">
                                        ...
                                        {isEditing ? (
                                            <div className="space-y-1.5">
                                                <input
                                                    ...
                                                    onChange={(e) => setWarehouseEditForm((prev) => ({ ...prev, address: e.target.value }))}
                                                    placeholder="Alamat"
                                                    className="w-full rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200"
                                                />
                                                <input
                                                    value={warehouseEditForm.phone}
                                                    onChange={(e) => setWarehouseEditForm((prev) => ({ ...prev, phone: e.target.value }))}
                                            ...
                                            <div className="space-y-0.5">
                                                {warehouse.address
                                                    ? <p className="text-xs text-gray-700 dark:text-gray-300 truncate max-w-[160px]">{warehouse.address}</p>
                                                    : <p className="text-xs text-gray-300 dark:text-gray-600">—</p>
                                                }
                                                {warehouse.phone && <p className="text-xs text-gray-400">{warehouse.phone}</p>}
                                            </div>
                                        )}
                                    </td>
                                    ...
                                    <td className="px-3 py-2.5">
                                        {isEditing ? (
                                            <div className="space-y-1.5">
                                                <input
                                                    value={warehouseEditForm.receipt_header}
                                                    onChange={(e) => setWarehouseEditForm((prev) => ({ ...prev, receipt_header: e.target.value }))}
                                                    placeholder="Header struk"
                                                    className="w-full rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-xs focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200"
                                                />
                                                <input
                                                    value={warehouseEditForm.receipt_footer}
                                                    onChange={(e) => setWarehouseEditForm((prev) => ({ ...prev, receipt_footer: e.target.value }))}
                                            ...
                                            <div className="space-y-0.5">
                                                {warehouse.receipt_header
                                                    ? <p className="text-xs text-gray-700 dark:text-gray-300 truncate max-w-[160px]" title={warehouse.receipt_header}>{warehouse.receipt_header}</p>
                                                    : <p className="text-xs text-gray-300 dark:text-gray-600">—</p>
                                                }
                                                {warehouse.receipt_footer && (
                                                    <p className="text-xs text-gray-400 truncate max-w-[160px]" title={warehouse.receipt_footer}>{warehouse.receipt_footer}</p>
                                                )}
                                            </div>
                                        )}
                                    </td>
