...
const emptyForm = () => ({ name: '', code: '', is_active: true, address: '', phone: '', receipt_header: '', receipt_footer: '' });

export function useWarehouseManagement() {
    const [warehouseForm, setWarehouseForm] = useState(emptyForm());
    ...
    const [warehouseEditForm, setWarehouseEditForm] = useState(emptyForm());
    ...
    const createWarehouse = async () => {
        ...
        payload.append('address', warehouseForm.address.trim());
        payload.append('phone', warehouseForm.phone.trim());
        payload.append('receipt_header', warehouseForm.receipt_header.trim());
        payload.append('receipt_footer', warehouseForm.receipt_footer.trim());

        if (warehouseCreateFile) {
            try {
                const compressed = await imageCompression(warehouseCreateFile, {
                    maxSizeMB: 0.35, maxWidthOrHeight: 1024, fileType: 'image/webp', initialQuality: 0.72, useWebWorker: false,
                });
        ...
        router.post('/settings/warehouses', payload, {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const startEditWarehouse = (warehouse: Warehouse) => {
        ...
        setWarehouseEditForm({
            ...
            receipt_header: warehouse.receipt_header ?? '',
            receipt_footer: warehouse.receipt_footer ?? '',
        });
    ...
    const cancelEditWarehouse = () => {
        ...
        setWarehouseEditForm(emptyForm());
    };
    ...
    const saveWarehouseUpdate = async () => {
        ...
        payload.append('address', warehouseEditForm.address.trim());
        payload.append('phone', warehouseEditForm.phone.trim());
        payload.append('receipt_header', warehouseEditForm.receipt_header.trim());
        payload.append('receipt_footer', warehouseEditForm.receipt_footer.trim());
        ...
        router.post(`/settings/warehouses/${editingWarehouseId}`, payload, {
            preserveScroll: true,
    ...
    const confirmDeleteWarehouse = () => {
        ...
        router.delete(`/settings/warehouses/${warehouseToDelete.id}`, {
            preserveScroll: true,
            onFinish: () => { setWarehouseLoading(false); closeDeleteWarehouseConfirm(); },
        });
    };

    return {
        warehouseForm, setWarehouseForm,
        warehouseLoading,
        createFileInputRef, setWarehouseCreateFile,
        editingWarehouseId,
        warehouseEditForm, setWarehouseEditForm,
        setWarehouseEditFile,
        deleteWarehouseConfirmOpen, warehouseToDelete,
        closeDeleteWarehouseConfirm, confirmDeleteWarehouse,
        createWarehouse, startEditWarehouse, cancelEditWarehouse, saveWarehouseUpdate,
        deleteWarehouse,
    };
}
