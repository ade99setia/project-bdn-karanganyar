...
export default function WerehouseSelect({ items, value, onChange, label, placeholder = 'Cari...', onPreviewImage, disabled = false }: Props) {
            ...
            <div className="relative">
                <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400" />
                    <input
                        ...
                        onFocus={() => { if (!disabled) setShowResults(true); }}
                        disabled={disabled}
                        className={`w-full h-12 pl-12 pr-4 bg-zinc-100 dark:bg-zinc-900 border-none rounded-2xl text-sm font-semibold transition-all ${disabled ? 'opacity-70 cursor-not-allowed' : 'focus:ring-2 focus:ring-indigo-500'}`}
                    />
            </div>
                        ...
                        {filtered.length > 0 ? (
                            filtered.map(i => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 p-2 rounded-xl transition-colors text-left group ${disabled ? 'opacity-70 cursor-not-allowed' : 'hover:bg-indigo-50 dark:hover:bg-indigo-900/20'}`}
                                >
                        ...
                        return (
                            <>
                                ...
                                <button
                                    ...
                                    className={`p-1.5 rounded-lg transition-colors text-indigo-600 ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-indigo-200 dark:hover:bg-indigo-900/40'}`}
                                >
                                    <X size={18} />
                                </button>
                            </>
